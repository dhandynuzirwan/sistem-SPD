                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    {{-- tombol eye untuk lihat file --}}
                                                    <a href="{{ route('letters.print', $letter->id) }}" class="btn btn-info mb-1" target="_blank">
                                                        <i class="la la-print"></i>
                                                    </a>
                                                    
                                                    @if($letter->status == 'pending')
                                                        <form action="{{ route('letters.approve', $letter->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyetujui surat ini?')">
                                                            @csrf
                                                            @method('PATCH')
                                                            <button type="submit" class="btn btn-primary mb-1">Approve</button>
                                                        </form>
                                                    @else
                                                        <button class="btn btn-secondary mb-1" disabled>Selesai</button>
                                                    @endif
                                                </td>   
                                            </tr>
                                            @empty
